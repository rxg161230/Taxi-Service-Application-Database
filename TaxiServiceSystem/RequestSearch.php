...
//print all the value which are in the array
  ...
  //	echo $v;
    $v = trim($v); 
  	...
  	$SRId_rs1 = mysqli_query($link,$sqll);
  	while($row = mysqli_fetch_array($SRId_rs1)){ 
	  $final_result [] = $row;
	}

 $sqll= "select * from servicerequest where SRId = '$SRId' GROUP BY SRId";
 $SRId_rs2 = mysqli_query($link,$sqll);
 while($row = mysqli_fetch_array($SRId_rs2)){ 
 	$final_result [] = $row;
	}

 $sqll= "select * from servicerequest where SRId = '$SRId' GROUP BY SRId";
 $SRId_rs3 = mysqli_query($link,$sqll);
  while($row = mysqli_fetch_array($SRId_rs3)){ 
 	$final_result [] = $row;
  }

 
 $sqll= "select * from servicerequest where CustomerNo = '$CustomerNo' GROUP BY CustomerNo";
  $CustomerNo_rs1 = mysqli_query($link,$sqll);
  while($row = mysqli_fetch_array($CustomerNo_rs1)){ 
 	$final_result [] = $row;
}

$sqll= "select * from servicerequest where CustomerNo = '$CustomerNo' GROUP BY CustomerNo";
 $CustomerNo_rs2 = mysqli_query($link,$sqll);
while($row = mysqli_fetch_array($CustomerNo_rs2)){ 
	  	$final_result [] = $row;
	}

$sqll= "select * from servicerequest where CustomerNo = '$CustomerNo' GROUP BY CustomerNo";
 $CustomerNo_rs3 = mysqli_query($link,$sqll);

while($row = mysqli_fetch_array($CustomerNo_rs3)){ 
	$final_result [] = $row;
}

$sqll= "select * from servicerequest where DriverNo = '$DriverNo' GROUP BY DriverNo";

$DriverNo_rs1 = mysqli_query($link,$sqll);
while($row = mysqli_fetch_array($DriverNo_rs1)){ 
...
$DriverNo_rs2 = mysqli_query($link,$sqll);

while($row = mysqli_fetch_array($DriverNo_rs2)){ 
...
$DriverNo_rs3 = mysqli_query($link,$sqll);

while($row = mysqli_fetch_array($DriverNo_rs3)){ 
